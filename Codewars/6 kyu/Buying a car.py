def nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth):
    ...
    months = 0
    left = 0
    currentFund = currentPriceOld
    while currentPriceNew > currentFund:
        currentPriceOld*=((100-percentLossByMonth)/100)
        currentPriceNew*=((100-percentLossByMonth)/100)
        saving+=savingperMonth
        currentFund=currentPriceOld+saving
        months+=1
        if months % 2 == 1:
            percentLossByMonth+=0.5
    left = currentFund - currentPriceNew
    return [months,round(left)]
